...
const QuestionCard = ({question, currentQuestionIndex, onSelect}: QuestionCardProps) => {
  ...
  return (
    <Fragment>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 gap-3 mt-5'>
          {currectQuestion.incorrect_answers.concat(currectQuestion.correct_answer).sort().map((answer, index) => (
              <Button className='h-full text-wrap' key={index} onClick={() => onSelect(answer)}>{answer}</Button>
          ))}
      </div>
    </Fragment>
  )
